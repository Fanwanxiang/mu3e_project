...
def fit_P_max(x,y0,err,func,xlabel,ylabel,title):
    ...
    plt.plot(x,theo_value, 'k--', lw=1, label='Pmax = 52.82')
    ...
    plt.xscale("log")
    plt.tight_layout()
    plt.savefig(fname =  'Fitted Pmax at various sample size', dpi=600)
    plt.show()
...
sample_size_l =np.concatenate((a3),axis = None)#, np.arange(10**3,2*10**3,10**2)
L =[]
for i in sample_size_l:
    P_max, err_P_max, Nmom_accept, ratio_accept_smaples = Accept_Reject_Mom(Nsmaples=i)
    L.append([P_max, err_P_max, Nmom_accept, ratio_accept_smaples])
#print(L)
...
#z_fitted_data  =  L
x = sample_size_l
y0 = np.transpose(L)[0] # Pmax
noise = np.transpose(L)[1] # Pmaxerr
Pmax,Pmax_err = fit_P_max(x,y0,noise,func,'sample size','P_max (MeV)','Fitted P_max at various sample size')
print(Pmax,Pmax_err )
# =============================================================================
# #%%
# N_mom_accept = len(mom_accept) # number of momentum samples 
