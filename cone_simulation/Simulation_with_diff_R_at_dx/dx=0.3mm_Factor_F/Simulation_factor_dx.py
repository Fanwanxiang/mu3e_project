...
def find_out_coincidences_3electrons_in_1_ns(N, dx, lmax, theta):
    # and return number of coincidence, 3 electtron
    # events and the
    # original events and so on.
    dy = dx
    dz = dx
    ...
    length = len(x3)  # length of the shortest array
    ...
    mask = (x12 < dx) & (x13 < dx) & (x23 < dx) & (y12 < dy) & (y13 < dy) & (y23 < dy) & (z12 < dz) & (z13 < dz) & (
            z23 < dz)
    x1_accept = x1[mask]
...
def main(program, factor, ds):  # ds is the spatial resolution
    ...
    n_in_dt = int(factor * R * dt)  # number of particles decays on the cone in dt
    ...
    filename = "factor=%.3f_dx=%.3fmm.txt" % (factor, ds)
    n_coincidence = find_out_coincidences_3electrons_in_1_ns(N=3 * n_in_dt, dx=ds, lmax=ls, theta=angle)
