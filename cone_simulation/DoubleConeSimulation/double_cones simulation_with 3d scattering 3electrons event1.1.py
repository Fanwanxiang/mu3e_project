...
def plot_3e(N1, lmax, theta):
    ...
    z2_accept = z2[mask] 
    z3_accept = z3[mask] 
    x_3e = np.concatenate([x1_accept, x2_accept, x3_accept])
    y_3e = np.concatenate([y1_accept, y2_accept, y3_accept])
    z_3e = np.concatenate([z1_accept, z2_accept, z3_accept])
    #allfalse = a = np.full(len(x1), False)
    #if mask.all() == false:
    #    print("there are no three points which are indistinguishable")
    #else:xyz = np.vstack([x,y,z])
    xyz = np.vstack([x_3e,y_3e,z_3e])
    kde = stats.gaussian_kde(xyz)
    density = kde(xyz)
    
    # Plot scatter with mayavi
    figure = mlab.figure('DensityPlot')
    pts = mlab.points3d(x_3e, y_3e, z_3e, density, scale_mode='none', scale_factor=0.07)
    mlab.axes()
    mlab.show()
    return x_3e, y_3e, z_3e
'''
figure = mlab.figure('DensityPlot')
...
x_3e, y_3e, z_3e = plot_3e(N1 = int(N_in_dt/2), lmax = 10, theta = np.pi/6)
plot_hist_1D(sample=x_3e, xlabel='x', ylabel='Pdf', title='3 electrons Probility distribution along x')
plot_hist_1D(sample=y_3e, xlabel='y', ylabel='Pdf', title='3 electrons Probility distribution along y')
plot_hist_1D(sample=x_3e, xlabel='z', ylabel='Pdf', title='3 electrons Probility distribution along z')
plot_hist_2D(sample1=x_3e, sample2=y_3e, xlabel='x',
             ylabel='y', title='3electrons 2D distribution on (x,y)',
             axis='equal')
plot_hist_2D(sample1=y_3e, sample2=z_3e, xlabel='y',
             ylabel='z', title='3electrons 2D distribution on (y,z)',
             axis='equal')
plot_hist_2D(sample1=x_3e, sample2=z_3e, xlabel='x',
