...
def ploynomial2(x,a):
    return a*x**4
def fit_exp(x,y): #fit N_3e -- dx space resolution
    ...
    plt.plot(x, func(x, *popt), 'r-',
              label='fit: a=%5.3f, b=%5.3f' % tuple(popt))#, c=%5.3f, d=%5.3f
    plt.xlabel('dx')
    plt.ylabel('prob')
    plt.legend()
    plt.show()
def fit_polynomial(x,y): #fit N_3e -- dx space resolution
    #xerr = np.random.random_sample(10)
    yerr = y_err
    fig, ax = plt.subplots()
    ...
    label='fit: a=%5.3f ' % tuple(popt))#,b=%5.3f, c=%5.3f, d=%5.3f
    plt.xlabel('dx (mm)')
    plt.ylabel('prob')
    plt.legend()
    plt.show()
...
def creat_arrays_of_prob_and_dx(n1,n2,n3,N): #n1 is the number of dx between 0.1-1
    L_dx1 = np.linspace(0.1,1,n1)            #N is the no. of simulations for each dx
    L_dx2 = np.linspace(1,5,n2)
    #L_dx3 = np.linspace(0.01, 0.05,5)
    L_dx3 = np.linspace(5,100,n3)
    L_dx = np.concatenate([L_dx1, L_dx2, L_dx3])
    L_y = []
    y_err = []
    ...
    return np.array(L_dx),np.array(L_y),np.array(y_err)

x,y,y_err = creat_arrays_of_prob_and_dx(n1=5,n2=10,n3=0,N=3)
'''plt.plot(x,y)
plt.xlabel("")
plt.ylabel("")
plt.title(title)
