...
R = 3*10**(8) # Ghz particles per unit time -- Particle Rate; upper limit of R that this laptop can simulate ~6*10^7
...
def creat_events_on_a_cone(N1, lmax, theta): # and return the original events #lmax is the slant length
...
def find_out_coincidences_3electrons_in_1_ns(N1, lmax, theta): # and return number of coincidence, 3electtron events and the original events and so on.
...
N_3e = find_out_coincidences_3electrons_in_1_ns(N1 = N_half, lmax = ls, theta=angle)
print(N_3e)
#2    
'''L = []
count = 0
